...
static SQInteger _blob_xor1(HSQUIRRELVM v)
{
	SETUP_BLOB(v);
	SQInteger num = self->Len();
	SQInteger mask;
	sq_getinteger(v, 2, &mask);
	unsigned short *t = (unsigned short *)self->GetBuf();
	for (SQInteger i = 0; i < num; i++) t[i] ^= mask;
	return 0;
}

unsigned char rol1(unsigned int value, unsigned char places) { return (value << places) | (value >> (8 - places)); } 
unsigned char ror1(unsigned int value, unsigned char places) { return (value >> places) | (value << (8 - places)); } 

static SQInteger _blob_rot1(HSQUIRRELVM v)
{
	SETUP_BLOB(v);
	SQInteger num = self->Len();
	SQInteger dir;
	unsigned char places;
	sq_getinteger(v, 2, &dir);
	unsigned char *t = (unsigned char *)self->GetBuf();
	// rotate left.
	if (dir < 0) {
		places = (-dir) % 8;
		for (SQInteger i = 0; i < num; i++) t[i] = rol1(t[i], places);
	}
	// rotate right.
	else {
		places = dir % 8;
		for (SQInteger i = 0; i < num; i++) t[i] = ror1(t[i], places);
	}
	return 0;
}
...
static SQRegFunction _blob_methods[] = {
	...
	_DECL_BLOB_FUNC(xor1,2,_SC("xn")),
	_DECL_BLOB_FUNC(rot1,2,_SC("xn")),
