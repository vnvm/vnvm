...
static double get_audio_clock(FFMovie *movie)
{
    ...
    bytes_per_sec = 0;
    if (movie->audio_st) {
        bytes_per_sec = movie->audio_st->codec->sample_rate * 2 * movie->audio_st->codec->channels;
    }
    if (bytes_per_sec)
...
static int queue_picture(FFMovie *movie, AVFrame *src_frame)
{
    ...
    if (movie->dest_overlay) {
        ...
        //img_convert(&pict, PIX_FMT_YUV420P, (AVPicture *)src_frame, movie->video_st->codec->pix_fmt, movie->video_st->codec->width, movie->video_st->codec->height);
		
		AVCodecContext *c = movie->video_st->codec;
		...
		//sws_scale(img_convert_ctx, pict.data, pict.linesize, 0, c->height, pict.data, pict.linesize);
		sws_scale(
			img_convert_ctx,
...
/*double get_pts_num_den(AVFormatContext *format_context, AVCodecContext *codec_context) {
	if (format_context->pts == AV_NOPTS_VALUE) {
		// AVRational.num.den || av_q2d
...
/*DECODE THREAD*/
    ...
    if (movie->video_st->codec->codec_id == CODEC_ID_RAWVIDEO) {
        avpicture_fill((AVPicture *)&frame, ptr, movie->video_st->codec->pix_fmt, movie->video_st->codec->width, movie->video_st->codec->height);
        if (pkt->pts != AV_NOPTS_VALUE) {
            pts = (double)pkt->pts * av_q2d(movie->video_st->codec->time_base);
        } else {
            pts = 0;
		}
        ...
        while (len > 0) {
            ...
            len1 = avcodec_decode_video(movie->video_st->codec, &frame, &got_picture, ptr, len);
            if (len1 < 0)
                break;
            if (got_picture) {
				movie->frame_count++; /*this should probably represent displayed frames, not decoded*/
                if (movie->vidpkt_timestamp != AV_NOPTS_VALUE) {
                    pts = (double)movie->vidpkt_timestamp * av_q2d(movie->video_st->codec->time_base);
                } else {
                    pts = 0;
				}
                update_video_clock(movie, &frame, pts);
...
static int synchronize_audio(FFMovie *movie, short *samples, int samples_size1, double pts)
{
/*SDL AUDIO THREAD*/
    int n, samples_size;
    double ref_clock;
...
/*SDL AUDIO THREAD*/
    ...
    for(;;) {
        ...
        while (movie->audio_pkt_size > 0) {
			//@TODO
            //len1 = avcodec_decode_audio(&movie->audio_st->codec, (int16_t *)audio_buf, &data_size, movie->audio_pkt_data, movie->audio_pkt_size);
			len1 = -1;
            if (len1 < 0) break;
            ...
            if (data_size > 0) {
                pts = 0;
                if (movie->audio_pkt_ipts != AV_NOPTS_VALUE) pts = (double)movie->audio_pkt_ipts * av_q2d(movie->audio_st->codec->time_base);
        ...
        if (pkt->destruct) av_free_packet(pkt);

        // read next packet
        if (packet_queue_get(&movie->audioq, pkt, 1) < 0) return -1;
...
/* MAIN THREAD */
    ...
    codec = avcodec_find_decoder(stream->codec->codec_id);
    if (!codec || avcodec_open(stream->codec, codec) < 0) return -1;

    movie->video_st = stream;
    movie->frame_last_delay = 40e-3;
    movie->frame_timer = SDL_GetTicks() / 1000.0;
	// @WARNING!!
    //movie->frame_delay = (double)movie->video_st->codec->frame_rate_base / (double)movie->video_st->codec->frame_rate;
...
/* MAIN THREAD */
    ...
    movie = (FFMovie *)av_mallocz(sizeof(FFMovie));
    if (!movie)
        return NULL;
