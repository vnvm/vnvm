...
import midi.LFO;
import midi.SamplerOpcodeGroup;
...
import midi.sf2.GeneratorEnum;
import midi.sf2.ModulatorType;
import midi.sf2.TransformEnum;
import midi.MIDITuning;
import midi.MathTools;
import haxe.ds.Vector;

typedef SF2Bank = Map<Int, SamplerOpcodeGroup>;
typedef MergedZone = { gen:Map<Int, Generator>, mod:Map<Int, Modulator> };
...
class SF2 
{
	...
	public var usable_banks:Map<Int, SF2Bank>;
	...
	private var sample_array : Array<{left:Array<Float>,right:Array<Float>,header:SampleHeader}>;
	private var reassigns : Map<Int, {from:Int,to:Int}>;
	...
	public function init(mip_up: Int, mip_down : Int, ?return_queue=false) : Array<Dynamic>
	{
		...
		reassigns = new Map<Int, {from:Int,to:Int}>();
		...
		queue.push(function() {
			usable_banks = new Map<Int, SF2Bank>();
			sample_idx = 0;
		});
	...
	{
		var gen_zone = new Map<Int, Generator>();
		var mod_zone = new Map<Int, Modulator>();
	...
	{
		var result_gen = new Map<Int, Generator>();
		var result_mod = new Map<Int, Modulator>();
				...
				for (g in preset.generators)
				{
					if (g.instrument != null)
					{
						...
						for (instrument in i_l)
						{
							...
							r.name = '${p.name}_${g.instrument.name}';
