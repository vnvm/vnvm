...
import midi.MIDITuning;
import midi.Envelope;
import haxe.io.Bytes;
import haxe.io.BytesInput;

typedef TString = midi.StringTools2;
typedef HString = StringTools;

typedef SFZOpcodeRegion = Map<String, SFZOpcodeInstance>;
...
typedef SFZEnvelopeDefinition = { delay:Float, start:Float, attack:Float, hold:Float, decay:Float, sustain:Float,
	release:Float, depth:Float, cc:Map<Int, Float>, vel2delay:Float, vel2attack:Float, vel2hold:Float, vel2decay:Float,
	vel2sustain:Float, vel2release:Float, vel2depth:Float };
typedef SFZLfoDefinition = { frequency:Float, depth:Float, delay:Float, attack:Float, freqcc:Map<Int, Float>,
	depthcc:Map<Int, Float>, depthaft:AftertouchDefinition, freqaft:AftertouchDefinition };
...
class SFZ
{
	...
	public function emitOpcodeGroup(seq : Sequencer, samples : Map<String, SamplerPatch>, ?use_cache=true) : SamplerOpcodeGroup
	{
		...
		for (r in regions)
		{
			...
			var ampeg_p : SFZEnvelopeDefinition = 
				{ delay:0., start:0., attack:0., hold:0., decay:0., sustain:1., release:0., depth:0.,
				  cc:new Map<Int, Float>(),
				  vel2delay:0., vel2attack:0., vel2hold:0., vel2decay:0., 
				  vel2sustain:0., vel2release:0., vel2depth:0. };
			var ampeg_cc = { delay:new Map<Int, Float>(), start:new Map<Int, Float>(), attack:new Map<Int, Float>(), hold:new Map<Int, Float>(),
				decay:new Map<Int, Float>(), sustain:new Map<Int, Float>(), release:new Map<Int, Float>()};
			...
			var amplfo_p : SFZLfoDefinition = { frequency:0., depth:0., delay:0., attack:0., 
				freqcc:new Map<Int, Float>(), depthcc:new Map<Int, Float>(), depthaft: { channel:0., poly:0. },
			...
			var filter_cutoff = 0.;
			var filter_cutoff_cc = new Map<Int, Float>();
