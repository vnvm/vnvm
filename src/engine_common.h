...
#include "sqstdlib/sqstdblobimpl.h"
...
#pragma pack(push, n, 1)
struct SVector2D { short x, y; };
struct SVectorQuad2D { SVector2D v[4]; };
#pragma pack(pop)

void printfunc(HSQUIRRELVM v, const SQChar *s, ...) { va_list vl; va_start(vl, s); vprintf(s, vl); va_end(vl); }
...
int screen_w_real = 320, screen_h_real = 480;
int screen_w = screen_w_real, screen_h = screen_h_real;
