...
class Interpolator {
    ...
    @:extern public static inline function interp_linear(a:Float, b:Float, x:Float) { return (a - (a - b) * x); }

// Recommendation: only use smoothstep for upsampling(lower pitch). It's too noisy for downsampling.

    @:extern public static inline function interp_smoothstep(a:Float, b:Float, x:Float) { return (a - (a - b) * x * x * (3 - 2 * x)); }
